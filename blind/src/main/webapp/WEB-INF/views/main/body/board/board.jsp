...
<div class="ui container">
	...
	<c:forEach var="post" items="${postList}">
		<article class="column" data-id="${post.postId}">
			<a class="ui grid row" href="${pageContext.request.contextPath}/post/${post.postId}" target="_blank">
...
<script>
	...
	<sec:authorize access="isAuthenticated()">
		fetch("${pageContext.request.contextPath}/addBookmark", {
			method: "POST",
			headers: {
	...
	addEventListener("DOMContentLoaded", function () {
		const getArticleHtml = function (post) {
			...
			html += "</a><div class=\"row\"><a href=\"${pageContext.request.contextPath}/companyIntroduction?" +
					"companyId=" + post.companyId + "\">" + post.companyName + "</a><span>・" + post.userNickname +
					"</span></div><div class=\"ui stakable two column grid row\"><div class=\"column\">" +
					"<span class=\"postInfo\"><i class=\"eye icon\"></i>" + post.postCount  + "</span>" +
					"<span class=\"postInfo\"><i class=\"thumbs up outline icon\"></i>" + post.postRecommendCount +
					"</span><span class=\"postInfo\"><i class=\"comment outline icon\"></i>" + post.replyCount +
					"</span></div><div class=\"right aligned column\"><span>" + post.postCreateDate +
		...
		let postId = postList.lastElementChild.dataset.id;
		const getNewArticle = async function () {
			await fetch("${pageContext.request.contextPath}/board/" + boardId + "?postId=" + postId, {
				method: "GET"
			}).then(function (response) {
				if (response.ok) {
					return response.json();
		...
		for (let i = slide.length - 1; i > -1; --i) {
			slide[i].addEventListener("click", async function () {
				...
				history.pushState({
					...
					postList: postList.innerHTML
				}, "", "${pageContext.request.contextPath}/board/" + boardId);
			});
		}
		...
		addEventListener("popstate", function (event) {
			...
			boardId = slide[slideIndex].dataset.id;

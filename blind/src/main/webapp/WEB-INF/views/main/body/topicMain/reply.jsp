...
<script>
											...
											$("#replyArea")
													.append(
															"<div class='ui container divider'></div>"
																	...
																	+ value.userId
																	+ "<br><br>"
																	+ "<div class = 'column' style='white-space:pre;'>"
																	+ value.replyContents
																	+ "</div>"
																	+ "<br>"
																	+ "<div class = 'column'>"
	...
	/* 	function replyList() {
	 ...
	 var $list = $("<li>");
	 var $userId = $("<span>").prop(
	 "class","userId").html(result[i].userId);
	 var $recCreateDate = $("<span>").prop(
	 "class","recCreateDate").html(result[i].recCreateDate);
	 var $replyContents = $("<p>").prop("class","replyContents")
					...
					<table class="table">
						<tr>
							<td><textarea style="width: 1100px" rows="3" cols="30"
									id="replyContents" name="replyContents" placeholder="댓글을 입력하세요"></textarea>
								<br>
								<div>
									<input type="button" id="replyButton" value="댓글 등록하기" />
								</div></td>
						</tr>
					</table>
				</div>

...
<script>
	...
	async function deleteFile(postFileId) {
		return await fetch("${pageContext.request.contextPath}/image", {
			method: "DELETE",
			headers: {
	...
	addEventListener("DOMContentLoaded", function () {
		...
		postUploadFiles.addEventListener("change", async function () {
			...
			for (let i = 0; i < filesLength; ++i) {
				...
				let postFile = await fetch(url.toString(), {
					method: "GET"
				}).then(function (response) {
					if (response.ok) {
				...
				if (postFile === null) {
					...
					url.search = "";
					postFile = await fetch(url.toString(), {
						method: "POST",
						body: formData
					}).then(function (response) {
						if (response.ok) {
			...
			editor.save().then(function (savedData) {
				...
				if (confirm("ポストを作成しますか？")) {
					...
					fetch("${pageContext.request.contextPath}/post", {
						...
						location.href = "${pageContext.request.contextPath}/post/" + postId;
					}).catch(function (error) {
						uploadingText.className = "ui disabled dimmer";

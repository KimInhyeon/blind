				...
				<script>
					...
					function setTableByCodeToName(tbody) {
						...
						if (tbody.childElementCount < 1) {
							tbody.innerHTML =
								"<tr><td class=\"center aligned\" colspan=\"11\">データが存在しません</td></tr>";
						}
					}
				...
				<c:choose>
					<c:when test="${navi.startPage gt navi.pagination}">
						<div class="ui pagination menu">
							<a class="item" href="javascript:getCompanyListByPage(${navi.startPage - navi.pagination})">
								前へ
							</a>
						</div>
					</c:when>
					<c:otherwise>
						<div class="ui pagination menu" style="visibility: hidden;">
							<a class="item">前へ</a>
						</div>
					</c:otherwise>
				</c:choose>
			...
			<div class="ui grid middle aligned">
				...
				<div class="row">
					...
					<div class="six wide column">
						...
						<c:forEach var="businessType" items="${businessTypeList}">
							<option value="${businessType.businessTypeCode}">
								${businessType.businessTypeCode}. ${businessType.businessTypeName}
							</option>
						</c:forEach>
						</select>
					</div>
